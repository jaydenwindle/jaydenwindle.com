import React from "react";

import Layout from "../components/layout";
import SEO from "../components/seo";

import { ArrowRight } from "react-feather";

function ContactPage() {
  return (
    <Layout>
      <SEO
        title="Consulting | Jayden Windle"
        keywords={[
          `consulting`,
          `Jayden Windle`,
          `software`,
          `software engineer`
        ]}
      />
      <section className="text-left mb-24">
        <h1 className="text-5xl font-bold inline-block my-8 lg:w-2/3">
          Let's bring your app idea to life.
        </h1>

        <p className="text-xl lg:w-2/3 mb-8">
          I help companies solve problems and drive growth by turning big ideas
          into world-class products.
        </p>

        <a
          href="https://calendly.com/jaydenwindle/intro-call?month=2020-01"
          rel="noopener noreferrer"
          target="_blank"
        >
          <button className="bg-gray-900 hover:bg-gray-800 hover:shadow-lg text-white font-bold py-2 px-4 rounded flex flex-row">
            Book an intro call
            <ArrowRight className="ml-4" />
          </button>
        </a>
      </section>

      <section className="text-left mb-24">
        <h3 className="text-2xl font-bold inline-block flex-grow mb-4">
          Who am I?
        </h3>
        <p className="text-xl mb-8">
          Hi, I'm Jayden! I'm a product-focused software engineer with over 6
          years of experience helping startups and established companies build
          amazing products that delight users and drive real growth.
        </p>
        <p className="text-xl mb-8">
          No matter what stage you're at in your project or what your
          requirements are, there's a good chance I can help. I've got
          experience managing and scaling development teams as a CTO, building
          beautiful products as a solo developer, and everything in between.
        </p>
      </section>

      <section className="text-left mb-24">
        <h3 className="text-2xl font-bold inline-block flex-grow mb-4">
          Companies I've worked with
        </h3>

        <div className="py-4">
          <div className="flex -mx-4 flex-col lg:flex-row justify-between">
            <div className="px-4 py-8 lg:py-0">
              <div className="flex items-center justify-center">
                <svg
                  className="h-16 max-w-full mx-auto"
                  style={{ maxWidth: "10rem" }}
                  // width="512"
                  // height="512"
                  viewBox="0 0 512 512"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <rect
                    width="512"
                    height="512"
                    rx="100"
                    fill="url(#paint1_linear)"
                  />
                  <path
                    fill-rule="evenodd"
                    clip-rule="evenodd"
                    d="M107 405.056V359.982V355.281C107 330.481 127.265 310.197 152.073 310.197V359.982H201.848C201.848 384.791 181.565 405.056 156.765 405.056H152.073H107Z"
                    fill="white"
                  />
                  <path
                    fill-rule="evenodd"
                    clip-rule="evenodd"
                    d="M391.671 159.487C362.152 108.353 296.823 90.8039 245.682 120.352C156.468 171.864 206.963 300.029 235.494 390.686C237.433 396.81 242.015 401.086 248.24 402.629C254.493 404.143 260.543 402.462 265.087 397.927L370.732 292.28C370.926 292.068 371.129 291.92 371.333 291.698C407.511 258.659 417.191 203.656 391.671 159.487ZM299.594 173.988C320.773 173.988 337.934 191.159 337.934 212.338C337.934 233.508 320.773 250.669 299.594 250.669C278.424 250.669 261.263 233.508 261.263 212.338C261.263 191.159 278.424 173.988 299.594 173.988"
                    fill="white"
                    fill-opacity="0.9"
                  />
                  <path
                    fill-rule="evenodd"
                    clip-rule="evenodd"
                    d="M352.524 120.33C403.666 149.859 421.178 215.17 391.659 266.304C340.148 355.556 211.975 305.023 121.32 276.51C115.197 274.561 110.92 269.98 109.378 263.773C107.863 257.529 109.553 251.461 114.079 246.916L219.734 141.288C219.946 141.084 220.094 140.881 220.297 140.659C253.363 104.508 308.329 94.8186 352.524 120.33ZM299.6 173.994C320.77 173.994 337.931 191.155 337.931 212.335C337.931 233.505 320.77 250.666 299.6 250.666C278.421 250.666 261.26 233.505 261.26 212.335C261.26 191.155 278.421 173.994 299.6 173.994Z"
                    fill="white"
                    fill-opacity="0.9"
                  />
                  <path
                    fill-rule="evenodd"
                    clip-rule="evenodd"
                    d="M371.996 134.688C379.564 141.75 386.226 150.038 391.671 159.47C394.038 163.565 396.101 167.754 397.869 172.01C408.596 197.831 408.437 226.119 398.942 251.032C396.95 256.221 394.526 261.321 391.659 266.287C355.376 329.155 281.057 322.668 208.606 303.374C189.317 230.927 182.844 156.617 245.682 120.335C249.502 118.127 253.403 116.182 257.361 114.495C273.289 107.759 290.75 104.911 308.159 106.375C319.762 107.37 331.137 110.254 341.806 114.896C345.436 116.483 349.014 118.287 352.525 120.314C359.687 124.449 366.19 129.286 371.996 134.688ZM337.934 212.321C337.934 191.142 320.773 173.971 299.594 173.971C288.518 173.971 278.539 178.671 271.541 186.187C265.162 193.034 261.261 202.22 261.261 212.318C261.261 221.1 264.214 229.192 269.182 235.656C276.189 244.775 287.205 250.652 299.594 250.652C320.773 250.652 337.934 233.491 337.934 212.321Z"
                    fill="white"
                  />
                  <defs>
                    <linearGradient
                      id="paint1_linear"
                      x1="0"
                      y1="512"
                      x2="512"
                      y2="0"
                      gradientUnits="userSpaceOnUse"
                    >
                      <stop stop-color="#313CFE" />
                      <stop
                        offset="1"
                        stop-color="#313CFE"
                        stop-opacity="0.8"
                      />
                    </linearGradient>
                  </defs>
                </svg>
              </div>
            </div>
            <div className="px-4 py-8 lg:py-0">
              <div className="flex items-center justify-center">
                <svg
                  className="h-16 max-w-full mx-auto"
                  style={{ maxWidth: "10rem", color: "#54317d" }}
                  id="svg"
                  viewBox="0, 0, 400,87.99999999999999"
                >
                  <g id="svgg">
                    <path
                      id="path0"
                      d="M164.833 0.675 C 156.682 1.598,152.097 3.886,150.208 7.970 C 149.185 10.181,149.086 60.095,150.099 62.719 C 152.325 68.488,162.611 71.481,176.438 70.383 C 184.075 69.777,188.098 68.516,190.973 65.826 C 194.003 62.992,193.833 64.800,193.833 35.333 L 193.833 9.500 192.899 7.597 C 190.185 2.071,178.324 -0.854,164.833 0.675 M259.351 0.707 C 254.024 1.325,250.200 2.773,248.051 4.988 C 246.030 7.069,245.833 7.959,245.833 15.000 C 245.833 23.661,245.073 22.537,259.295 34.904 C 274.070 47.752,273.335 46.700,273.331 54.977 C 273.328 63.359,271.324 65.562,264.363 64.837 C 258.800 64.258,258.003 63.141,258.001 55.917 L 258.000 50.667 251.500 50.667 L 245.000 50.667 245.000 56.019 C 245.000 62.242,245.306 63.793,246.906 65.662 C 252.402 72.082,276.397 72.383,283.344 66.120 C 285.821 63.887,285.984 63.198,285.993 54.976 C 286.004 44.183,286.717 45.309,271.786 32.543 C 257.850 20.628,258.543 21.636,258.723 13.555 L 258.833 8.610 259.883 7.673 C 262.399 5.427,269.404 5.455,271.669 7.720 L 272.667 8.718 272.667 13.859 L 272.667 19.000 279.167 19.000 L 285.667 19.000 285.659 13.583 C 285.650 6.489,284.925 4.822,280.979 2.823 C 276.833 0.722,267.285 -0.212,259.351 0.707 M352.018 0.707 C 346.710 1.322,342.832 2.808,340.640 5.068 C 338.703 7.063,338.500 8.007,338.500 15.000 C 338.500 23.774,337.522 22.365,352.943 35.808 C 366.719 47.817,365.979 46.715,365.992 55.228 C 366.005 63.349,364.783 64.842,358.167 64.796 C 351.726 64.752,350.670 63.501,350.668 55.917 L 350.667 50.667 344.140 50.667 L 337.614 50.667 337.724 56.750 C 337.846 63.421,338.094 64.383,340.219 66.398 C 346.283 72.145,369.522 71.980,375.982 66.143 C 378.569 63.806,378.668 63.388,378.659 54.763 C 378.649 44.056,379.671 45.653,364.423 32.519 C 350.756 20.746,351.538 21.818,351.402 14.667 C 351.259 7.151,352.055 6.199,358.500 6.184 C 364.683 6.170,365.447 7.017,365.619 14.068 L 365.739 19.000 372.036 19.000 L 378.333 19.000 378.326 13.583 C 378.316 6.406,377.641 4.854,373.643 2.823 C 369.508 0.723,359.954 -0.212,352.018 0.707 M12.724 1.150 C 3.221 2.946,1.329 5.246,1.341 14.982 C 1.351 23.954,0.466 22.640,14.972 35.228 C 29.711 48.018,28.759 46.585,28.605 55.749 L 28.500 61.958 27.509 62.991 C 25.409 65.182,19.157 65.717,15.986 63.978 C 13.649 62.696,13.334 61.741,13.334 55.917 L 13.333 51.000 7.000 51.000 L 0.667 51.000 0.674 56.917 C 0.682 63.748,0.944 64.710,3.345 66.766 C 8.959 71.571,28.612 72.137,36.785 67.729 C 41.057 65.424,41.778 63.376,41.614 54.000 C 41.444 44.296,42.225 45.449,27.000 32.420 C 13.141 20.561,14.333 22.260,14.333 14.370 C 14.333 6.971,15.201 6.077,22.072 6.397 C 27.711 6.660,28.333 7.480,28.333 14.656 L 28.333 19.333 34.833 19.333 L 41.333 19.333 41.333 14.683 C 41.333 5.874,39.784 3.502,32.805 1.624 C 29.378 0.703,16.678 0.403,12.724 1.150 M43.343 1.417 C 43.356 1.943,60.156 54.600,60.384 54.828 C 60.477 54.921,61.989 50.490,63.746 44.982 L 66.939 34.967 61.553 17.987 L 56.167 1.006 49.750 1.003 C 44.832 1.001,43.336 1.097,43.343 1.417 M80.000 1.168 C 80.000 1.260,73.850 20.601,66.333 44.146 C 58.817 67.692,52.667 87.117,52.667 87.312 C 52.667 87.551,54.765 87.665,59.083 87.661 L 65.500 87.656 79.167 44.802 C 86.683 21.232,92.882 1.735,92.941 1.474 C 93.031 1.077,91.994 1.000,86.524 1.000 C 82.936 1.000,80.000 1.076,80.000 1.168 M96.667 35.471 L 96.667 70.000 103.000 70.000 L 109.333 70.000 109.333 38.303 L 109.333 6.605 117.250 6.725 C 124.546 6.835,125.292 6.902,126.761 7.575 C 129.499 8.830,129.333 6.718,129.333 40.323 L 129.333 70.000 135.673 70.000 L 142.013 70.000 141.923 38.890 L 141.833 7.781 140.820 6.140 C 138.165 1.839,135.143 1.276,113.750 1.091 L 96.667 0.943 96.667 35.471 M200.667 35.471 L 200.667 70.000 207.000 70.000 L 213.333 70.000 213.333 54.540 L 213.333 39.079 221.917 38.900 C 240.959 38.501,241.658 37.831,241.663 19.982 L 241.667 8.464 240.800 6.725 C 238.435 1.980,235.343 1.289,215.583 1.091 L 200.667 0.942 200.667 35.471 M287.855 1.750 C 288.585 4.101,303.469 50.855,304.047 52.613 C 304.948 55.357,304.769 55.698,308.321 44.466 L 311.389 34.766 306.677 19.966 C 304.085 11.826,301.660 4.229,301.288 3.083 L 300.612 1.000 294.117 1.000 L 287.622 1.000 287.855 1.750 M310.733 44.083 C 303.176 67.779,296.995 87.279,296.997 87.417 C 296.998 87.554,299.891 87.667,303.424 87.667 L 309.848 87.667 323.458 44.990 C 330.943 21.518,337.145 2.018,337.239 1.657 C 337.406 1.020,337.215 1.000,330.942 1.000 L 324.473 1.000 310.733 44.083 M389.238 1.791 C 384.129 3.217,381.745 9.380,384.548 13.914 C 387.812 19.194,396.210 18.869,398.979 13.356 C 402.213 6.918,396.247 -0.164,389.238 1.791 M394.783 4.108 C 398.150 5.993,398.879 10.883,396.241 13.887 C 392.427 18.232,385.312 15.463,385.341 9.645 C 385.366 4.604,390.397 1.653,394.783 4.108 M388.333 9.667 L 388.333 14.333 389.167 14.333 C 389.963 14.333,390.000 14.244,390.000 12.333 C 390.000 9.564,390.817 9.564,392.527 12.333 C 393.570 14.023,393.909 14.333,394.714 14.333 C 395.907 14.333,395.907 14.331,394.504 12.248 C 393.454 10.688,393.398 10.493,393.921 10.219 C 394.935 9.690,395.333 8.911,395.333 7.455 C 395.333 5.432,394.588 5.000,391.092 5.000 L 388.333 5.000 388.333 9.667 M177.148 6.724 C 178.819 7.267,180.505 8.943,180.784 10.337 C 181.393 13.382,180.998 60.139,180.351 61.564 C 178.225 66.246,164.768 66.071,162.873 61.336 C 162.393 60.135,162.333 57.285,162.333 35.397 L 162.333 10.808 163.083 9.340 C 164.603 6.366,171.882 5.012,177.148 6.724 M393.022 6.678 C 394.597 7.521,393.295 9.000,390.978 9.000 C 390.059 9.000,390.000 8.919,390.000 7.667 C 390.000 6.229,391.354 5.786,393.022 6.678 M226.428 7.586 C 228.936 8.718,229.000 9.026,229.000 20.000 C 229.000 33.290,229.177 33.054,219.083 33.274 L 213.333 33.399 213.333 19.995 L 213.333 6.591 219.083 6.729 C 224.076 6.848,225.043 6.961,226.428 7.586 "
                      stroke="none"
                      fill-rule="evenodd"
                      className="fill-current"
                    ></path>
                  </g>
                </svg>
              </div>
            </div>
            <div className="px-4 py-8 lg:py-0">
              <div className="items-center justify-center">
                <svg
                  className="h-16 max-w-full mx-auto text-gray-500"
                  style={{ maxWidth: "10rem" }}
                  viewBox="0 0 39 36"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    fill-rule="evenodd"
                    clip-rule="evenodd"
                    d="M8.64154 15.2986V23.1599L12.3602 25.8087V17.3801L8.64154 15.2986Z"
                    fill="#1D3957"
                  />
                  <path
                    fill-rule="evenodd"
                    clip-rule="evenodd"
                    d="M8.65359 23.1526L8.64154 27.9752L12.3602 30.6836V25.7966L8.65359 23.1526Z"
                    fill="#3B3C39"
                  />
                  <path
                    fill-rule="evenodd"
                    clip-rule="evenodd"
                    d="M8.65359 23.1526V27.9631L0.163994 32.8567V28.0054L8.65359 23.1526Z"
                    fill="#A8A9A5"
                  />
                  <path
                    fill-rule="evenodd"
                    clip-rule="evenodd"
                    d="M8.65359 27.9631L12.358 30.6823L4.34728 35.3266L0.163994 32.8567L8.65359 27.9631Z"
                    fill="#4E4E4B"
                  />
                  <path
                    fill-rule="evenodd"
                    clip-rule="evenodd"
                    d="M19.4414 14.4913L0.163994 3.1729V28.0054L8.65359 23.1527V15.3154L19.4414 21.3596V14.4913Z"
                    fill="#2E6EA0"
                  />
                  <path
                    fill-rule="evenodd"
                    clip-rule="evenodd"
                    d="M19.4414 14.4917L38.7173 3.17422L30.2489 0.0440979L19.4407 6.19819L8.63238 0.0440979L0.163994 3.17422L19.4399 14.4917V14.4925L19.4407 14.4921L19.4414 14.4925V14.4917Z"
                    fill="#498BBD"
                  />
                  <path
                    fill-rule="evenodd"
                    clip-rule="evenodd"
                    d="M30.2289 15.3142V23.1647L26.5211 25.8087V17.3898L30.2289 15.3142Z"
                    fill="#1D3957"
                  />
                  <path
                    fill-rule="evenodd"
                    clip-rule="evenodd"
                    d="M30.2289 23.1526L30.241 27.9752L26.5211 30.6836V25.7966L30.2289 23.1526Z"
                    fill="#3B3C39"
                  />
                  <path
                    fill-rule="evenodd"
                    clip-rule="evenodd"
                    d="M30.2277 23.1526V27.9631L38.7173 32.8567V28.0054L30.2277 23.1526Z"
                    fill="#676864"
                  />
                  <path
                    fill-rule="evenodd"
                    clip-rule="evenodd"
                    d="M30.2277 27.9631L26.5233 30.6823L34.534 35.3266L38.7173 32.8567L30.2277 27.9631Z"
                    fill="#4E4E4B"
                  />
                  <path
                    fill-rule="evenodd"
                    clip-rule="evenodd"
                    d="M19.4399 14.4913L38.7173 3.1729V28.0054L30.2277 23.1527V15.3154L19.4399 21.3596V14.4913Z"
                    fill="#27527B"
                  />
                </svg>
              </div>
            </div>
            <div className="px-4 py-8 lg:py-0">
              <div className="flex items-center justify-center">
                <svg
                  className="h-16 max-w-full mx-auto text-gray-500"
                  style={{ maxWidth: "10rem", color: "#ed1c24" }}
                  viewBox="0 0 355.12 36"
                >
                  <g id="Layer_2" data-name="Layer 2">
                    <g id="Layer_1-2" data-name="Layer 1">
                      <rect
                        y="0.68"
                        width="7.36"
                        height="34.64"
                        className="fill-current"
                      />
                      <path
                        d="M44.79,12.12c0,2.88,0,5.72,0,7.83,0,1.07,0,2,0,2.63l-1-1.43c-1.47-2-3.7-5.15-5.16-7.07L28.7.68H21.88V35.32h7.36V24c0-3.55,0-6.6,0-8.68,0-.87,0-1.64,0-2.17L30.46,15c1.41,2,3.35,4.7,5.1,7.09l9.76,13.25h6.82V.68H44.79Z"
                        className="fill-current"
                      />
                      <path
                        d="M89.57,12.12c0,2.88,0,5.72,0,7.83,0,1.07,0,2,0,2.63l-1-1.43c-1.47-2-3.7-5.15-5.16-7.07L73.48.68H66.66V35.32H74V24c0-3.55,0-6.6,0-8.68,0-.87,0-1.64,0-2.17L75.24,15c1.41,2,3.35,4.7,5.1,7.09L90.1,35.32h6.82V.68H89.57Z"
                        className="fill-current"
                      />
                      <path
                        d="M126.87,28.74c-6.07,0-10.31-4.42-10.31-10.74S120.8,7.26,126.87,7.26,137.22,11.68,137.22,18,133,28.74,126.87,28.74m0-28.74a17.58,17.58,0,0,0-18,18,18,18,0,1,0,36,0,17.61,17.61,0,0,0-18-18"
                        className="fill-current"
                      />
                      <path
                        d="M171,21.5c-.81,2.23-1.38,4.09-1.69,5.09v0l-.12-.36c-.32-1-.87-2.76-1.57-4.77L159.75.68h-8l13.81,34.64h7.6L186.92.68h-8Z"
                        className="fill-current"
                      />
                      <polygon
                        points="227.61 7.6 238.21 7.6 238.21 35.32 245.57 35.32 245.57 7.6 256.27 7.6 256.27 0.68 227.61 0.68 227.61 7.6"
                        className="fill-current"
                      />
                      <rect
                        x="267.43"
                        y="0.68"
                        width="7.36"
                        height="34.64"
                        className="fill-current"
                      />
                      <path
                        d="M304.16,21.5c-.8,2.23-1.37,4.09-1.68,5.09a0,0,0,0,0,0,0c0-.11-.07-.24-.11-.36-.33-1-.88-2.76-1.57-4.77L292.88.68h-8l13.81,34.64h7.6L320.06.68h-8Z"
                        className="fill-current"
                      />
                      <polygon
                        points="330.16 0.68 330.16 35.32 355.12 35.32 355.12 28.4 337.51 28.4 337.51 20.78 351.18 20.78 351.18 14.25 337.51 14.25 337.51 7.6 354.83 7.6 354.83 0.68 330.16 0.68"
                        className="fill-current"
                      />
                      <path
                        d="M209.78.68H203L203,.83l3.42,9.23a.11.11,0,0,0,0-.05l.17.48c.07.22.15.45.23.69l1.88,5.07h0l2.33,6H201.8l2.33-6H196.6l-7.84,19.07h7.94l2.53-6.52h14.33l2.53,6.52H224Z"
                        className="fill-current"
                      />
                    </g>
                  </g>
                </svg>
              </div>
            </div>
            <div className="px-4 py-8 lg:py-0">
              <div className="flex items-center justify-center">
                <svg
                  className="h-16 max-w-full mx-auto"
                  style={{ maxWidth: "10rem" }}
                  viewBox="0 0 77 89"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M38.18 89C29.55 89 20.13 85.65 14.04 76.14C7.68 66.2 2.2 51.4 0.369998 39.26C-1.53 26.62 4 15.33 14.77 9.8C22.3186 5.97153 30.2804 3.01901 38.5 0.999998C51.1 -2.12 62.1 2.3 67.94 12.8C71.06 18.38 74.54 29.5 75.94 35.28C78.64 46.55 74.24 56.65 64.48 61.64C60.6 63.64 56.48 65.01 48.18 67.54C39.33 70.24 31.18 67.7 27.46 61.04C24.54 55.84 21.06 47.31 20.04 41.04C19.4628 37.5733 20.0285 34.0128 21.652 30.8958C23.2754 27.7788 25.8685 25.2742 29.04 23.76C34.641 21.1983 40.4655 19.1564 46.44 17.66L46.54 18.06C47.0522 20.3203 46.6973 22.6905 45.5454 24.7016C44.3936 26.7127 42.5288 28.2181 40.32 28.92C37.7938 29.709 35.3275 30.6781 32.94 31.82C31.5001 32.4959 30.3204 33.6236 29.5803 35.0316C28.8403 36.4396 28.5803 38.0507 28.84 39.62C29.58 44.15 32.34 51.47 35.24 56.62C37.79 61.15 44.74 59.19 45.52 58.95C54.2 56.3 57.39 55.15 60.37 53.63C66.37 50.57 68.9 44.48 67.19 37.33C65.69 31.13 62.39 21.33 60.09 17.13C57.79 13.13 52.29 6.79 40.59 9.67C33.038 11.529 25.7237 14.2467 18.79 17.77C11.32 21.6 7.79 28.95 9.19 37.93C10.79 48.76 15.89 62.48 21.52 71.3C26.96 79.8 36.06 80.7 43.68 79.3C49.8472 77.9716 55.8867 76.1079 61.73 73.73C64.0512 72.85 66.6263 72.9238 68.8932 73.9355C71.1602 74.9472 72.9349 76.8146 73.83 79.13C65.68 82.13 55.45 85.91 45.7 88.17C43.2062 88.7332 40.6566 89.0117 38.1 89H38.18Z"
                    fill="url(#paint0_linear)"
                  />
                  <defs>
                    <linearGradient
                      id="paint0_linear"
                      x1="3.2885e-07"
                      y1="45"
                      x2="77"
                      y2="45"
                      gradientUnits="userSpaceOnUse"
                    >
                      <stop stop-color="#A60063" />
                      <stop offset="1" stop-color="#552A7A" />
                    </linearGradient>
                  </defs>
                </svg>
              </div>
            </div>
          </div>
        </div>
      </section>

      <section className="text-left mb-24">
        <h2 className="text-4xl font-bold inline-block my-8 lg:w-2/3">
          Ready to get started?
        </h2>

        <a
          href="https://calendly.com/jaydenwindle/intro-call?month=2020-01"
          rel="noopener noreferrer"
          target="_blank"
        >
          <button className="bg-gray-900 hover:bg-gray-800 hover:shadow-lg text-white font-bold py-2 px-4 rounded flex flex-row">
            Book an intro call
            <ArrowRight className="ml-4" />
          </button>
        </a>
      </section>
    </Layout>
  );
}

export default ContactPage;
